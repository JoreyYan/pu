...
def _process_csv_row(processed_file_path, map, base_thickness):
    ...
    # [Old] from_rigid_and_sidechain: 只用侧链原子
    ...
    # [New] from_all_atoms: 使用全原子质心（包括骨架N,CA,C,O + 侧链）
    ...
    scaing_log_1 = rigids_1._scaling_log
   ...
   # analyze_your_data(aligned_frame.unsqueeze(0))
    ...
    return {
        'scaing_log_1':scaing_log_1.detach(),
...
class BaseDataset(Dataset):
    ...
    def process_csv_row(self, csv_row):
        ...
        processed_row = _process_csv_row(path, self.map, self.base_thickness)
        if use_cache:
