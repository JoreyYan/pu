...
class System:
    ...
    def from_XCS(
        ...
        """
        alphabet = constants.AA20+'X' if alternate_alphabet is None else alternate_alphabet
    ...
    def from_XCSB(
        ...
        assert B.shape[0] == 1
        assert X.shape[1] == S.shape[1]
        assert C.shape[1] == S.shape[1]
        assert B.shape[1] == S.shape[1]

        X, C, S,B = [T.squeeze(0).cpu().data.numpy() for T in [X, C, S,B]]
        ...
        for i, chain_id in enumerate(np.unique(chain_ids)):
            ...
            chain_bool = chain_ids == chain_id
            X_chain = X[chain_bool, :, :].tolist()
            B_chain = B[chain_bool, :].tolist()
            C_chain = C[chain_bool].tolist()
            S_chain = S[chain_bool].tolist()

            # Build chain
            chain = new_system.add_chain("A")
            for chain_ix, (X_i, C_i, S_i,B_i) in enumerate(zip(X_chain, C_chain, S_chain,B_chain)):
                resname = polyseq.to_triple(alphabet[int(S_i)])
                ...
                if C_i > 0:
                    ...
                    for atom_ix, atom_name in enumerate(atom_names):
                        ...
                        bf_i=B_i[atom_ix]
                        atom_count += 1
                        residue.add_atom(atom_name, False, x, y, z, 1.0, bf_i, " ")
