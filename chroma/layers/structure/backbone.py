...
class FrameBuilder(nn.Module):
    ...
    def _build_O(self, X_chain: torch.Tensor, C: torch.LongTensor):
        ...
        X_O = geometry.extend_atoms(
            ...
            X_CA,
            X_C,
            (self._length_C_O * ones).to(X_chain.device),
            (self._angle_CA_C_O * ones).to(X_chain.device),
            (self._dihedral_Np_CA_C_O * ones).to(X_chain.device),
            degrees=True,
        )
    ...
    def forward(
        ...
        X_chain = transforms.compose_translation(R, t_frame, self._t_atom.to(R.device))
