...
class BackboneMVNGlobular(torch.nn.Module):
    ...
    def _multiply_R(self, Z, C):
        ...
        if self.covariance_model == "brownian":
            ...
            # 将张量转换为 numpy 数组
            points = R_Z_expand.view(-1, 3).detach().cpu().numpy()
            with open("points_z.pdb", "w") as file:
                for i, (x, y, z) in enumerate(points):
                    file.write(f"HETATM{i + 1:5d}  C   UNK     1    {x:8.3f}{y:8.3f}{z:8.3f}\n")
    ...
    def sample(
        ...
        if Z is None:
            Z = torch.randn([num_batch, num_residues * 1, 3], device=C.device)
        if ddX is None:
        ...
        X = X_flat.reshape([num_batch, num_residues, 1, 3])
        return X
...
class BackboneMVNResidueGas(torch.nn.Module):
    ...
    def _register(self,stddev_CA=10.0, stddev_atoms=1.0,device='cuda'):
        ...
        self.register_buffer("R_local", R_local.to(device))
        self.register_buffer("Ri_local", torch.linalg.inv(R_local).detach().to(device))
    ...
    def _multiply_R(self, Z, C):
        ...
        if torch.any(torch.isnan(Z_unflat)):
            raise ValueError('nan in Z_unflat')

        if torch.any(torch.isinf(Z_unflat)):
            raise ValueError(' inf in Z_unflat')

        R_Z_unflat = torch.einsum("biax,ca->bicx", Z_unflat, self.R_local)
        if torch.any(torch.isnan(R_Z_unflat)):
            print(Z_unflat)
            raise ValueError('nan in R_Z_unflat')
    ...
    def test_sample(self,
        ...
        if Z is None:
            ...
            noCa_Z = torch.randn([num_batch, num_residues * 4, 3], device=C.device)
            n,ca,c,o=torch.unbind(noCa_Z, dim=-1)
            ca=ca*0+Z
            Z=torch.stack([n,ca,c,o], dim=-2)
...
if __name__ == '__main__':
    ...
    bb=BackboneMVNGlobular()
    C=torch.ones([1,10], dtype=torch.long)
    X=bb.sample(C)
    print(X.shape)