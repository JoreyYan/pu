...
from models.shattetnion.ShDecoderSidechain import \
    NodeFeatExtractorWithHeads  # ,SHSidechainDecoder,DynamicKSidechainDecoder,SHTypeHybridHead,SHGeoResHead,assemble_atom14
...
class VAE(nn.Module):
    ...
    def forward(self, input_feats, infer_only=False):
        if infer_only:
            ...
            z_q, downsampled_mask, log_dict = self.PCT.encode(x, mask=input_feats['res_mask'], verbose=False,
                                                              infer_only=False)
...
class SHframe_fbb(nn.Module):
    def __init__(self, model_conf):
        ...
        self.SH_embedding = SHFeatureHead(C=model_conf.sh.C, L_max=model_conf.sh.L_max, R_bins=model_conf.sh.R_bins,
                                          hidden=model_conf.sh.hidden_scalar, num_classes=-1,
                                          dropout=model_conf.sh.dropout)
...
class SideAtomsFlowModel_backup(nn.Module):
    ...
    def forward(self, input_feats, t_set=None):
        ...
        speed_pred = side_atoms.norm(dim=-1)

        return {
            'speed_vectors': side_atoms,
