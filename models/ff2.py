...
from models.ff2flow.adapters import (
...
from models.ff2flow.structure_network import FF2StructureNetwork
from models.ff2flow.trunk import FF2TrunkTransformer
from models.components.sequence.frozen_esm import FrozenEsmModel
from openfold.utils import rigid_utils as ru
from torch import nn
from models.components.sequence.frozen_esm import ESM_REGISTRY
from models.utils import RelativePosition
from models.se3_fm import SE3FlowMatcher
...
class FF2Model(nn.Module):
    ...
    #     }
    #     # # replace esm model name to 12layer
    #     # ckpt['esm_model']=deps.config.model.esm2_model_key
    #
    ...
    def from_ckpt(cls, ckpt: Dict[str, torch.Tensor], deps: FF2Dependencies):
        ...
        # 不再检查 ESM 一致性
        # ckpt_lm_name = ckpt.get("esm_model")

        model = cls.from_dependencies(deps)
        # do not load
        # # 关键：在 load 之前剥离 ESM 及其依赖层的参数
        # cls._strip_esm_and_dependents(ckpt)
        #
        # # 允许部分缺失（因为我们刚删除了一些 key）
        # missing, unexpected = model.load_state_dict(ckpt["state_dict"], strict=False)
        # 你也可以打印/记录 missing、unexpected 以确认安全
        return model
    ...
    def _strip_esm_and_dependents(ckpt: Dict[str, any]) -> None:
        sd = ckpt["state_dict"]

        # 1) 删掉 ESM 本体权重
        for k in [k for k in list(sd.keys()) if k.startswith("seq_encoder.")]:
            sd.pop(k)

        # 2) 删掉强依赖 ESM 维度的子网（你项目里常见命名，按需增删）
        #    例如把 ESM 的 [B,L,D_esm] 投到干线的投影层：
        for prefix in [
            "sequence_to_trunk_network.",

        ]:
            for k in [k for k in list(sd.keys()) if k.startswith(prefix)]:
                sd.pop(k)

        # 3) 可选：去掉记录用字段
        ckpt.pop("esm_model", None)
    ...
    def forward(
        ...
        single_representation = {"bb": bb_emb_s, "seq": torch.zeros((bb_emb_s.shape[0], bb_emb_s.shape[1], 128))}
        pair_representation = {"bb": bb_emb_z, "seq": torch.zeros_like(bb_emb_z)}
