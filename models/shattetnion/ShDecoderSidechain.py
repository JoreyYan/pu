...
class Feat2Atom11(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.0,
        out_range: float = 8.0,  # tanh 缩放到 ±out_range Å
    ):
        super().__init__()
        self.out_range = out_range
...
class SHGeoResHead(nn.Module):
    ...
    结构:
      ...
      - 融合: concat 两支，再 MLP 输出
    """
    def __init__(self, C, L_max, R_bins, hidden=256, num_classes=21, dropout=0.1,**kwargs):
        super().__init__()
        self.C, self.L_max, self.R = C, L_max, R_bins

        self.SH_embedding=SHFeatureHead(C, L_max, R_bins, hidden=hidden, num_classes=num_classes, dropout=dropout)
        #self.Torsion_Pred=AngleResnet(c_in=hidden, c_hidden=hidden, no_blocks=2, no_angles=7, epsilon=1e-9)
    ...
    def forward(self, SH: torch.Tensor, node_mask: torch.Tensor | None = None):
        ...
        aalogits,feat=self.SH_embedding(SH,node_mask)
        # un_tor_pred, tor_pred=self.Torsion_Pred(feat)
        coords=self.atoms14_Head(feat)


        return aalogits,coords  #un_tor_pred, tor_pred
