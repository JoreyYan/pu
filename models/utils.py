...
def t_stratified_mean_loss(batch_t, batch_loss, loss_name=None):
    """Compute overall average loss without stratifying by t bins."""
    batch_t = du.to_numpy(batch_t)
    batch_loss = du.to_numpy(batch_loss)
    ...
    # Compute overall average loss (optionally weighted by t, or just mean)
    total_loss = np.mean(flat_losses)

    stratified_losses = {f'{loss_name}_mean': total_loss}
    return stratified_losses
