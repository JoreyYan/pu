...
class FlowModule(LightningModule):
    ...
    def predict_step(self, batch, batch_idx):
        ...
        if diagnostics:
            ...
            if 'sidechain_rmsd' in diagnostics:
                diag_str += f"Sidechain_RMSD={diagnostics['sidechain_rmsd']:.4f}Å  "
        ...
        for b, sample_id in enumerate(sample_ids):
            ...
            if diagnostics:
                ...
                with open(diag_path, 'w') as f:
                    ...
                    if 'sidechain_rmsd' in diagnostics:
                        f.write(f"Sidechain RMSD (vs GT): {diagnostics['sidechain_rmsd']:.4f} Å\n\n")
    ...
    def _save_structures(self, batch, batch_idx, sample_out, method_name='ode'):
        """
        保存结构文件的通用函数
    ...
    def predict_stepodesde(self, batch, batch_idx):
        ...
        log_msg = (
            ...
            f"ODE_RMSD={rmsd_ode:.4f}Å, SDE_RMSD={rmsd_sde:.4f}Å, "
            f"Diff={(rmsd_sde - rmsd_ode):+.4f}Å "
        ...
        if (batch_idx + 1) % 10 == 0:
            ...
            stats_msg = (
                ...
                f"  ODE avg RMSD: {np.mean(ode_rmsds):.4f} ± {np.std(ode_rmsds):.4f} Å\n"
                f"  SDE avg RMSD: {np.mean(sde_rmsds):.4f} ± {np.std(sde_rmsds):.4f} Å\n"
                f"  Difference: {(np.mean(sde_rmsds) - np.mean(ode_rmsds)):+.4f} Å"
            )
