...
from models.hours._modules import HourglassDecoder, HourglassEncoder, VectorQuantizer, FiniteScalarQuantizer
from models.quantize import VectorQuantizerEMA
from models.cheap.utils import (
    LatentScaler,
...
class HourglassProteinCompressionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        e_dim=64,
        vq_beta=0.25,
        enforce_single_codebook_per_position: bool = True,
        ...
        # 计算总的缩减倍数
        if isinstance(downproj_factor, (list, tuple,ListConfig)):
            total_downproj = np.prod(downproj_factor)
        else:
            total_downproj = downproj_factor
        ...
        if isinstance(use_quantizer, bool):
            if use_quantizer:
                ...
                print('quantizer params:',n_e, e_dim, vq_beta)
        ...
        if self.quantize_scheme == "vq":
            self.quantizer = VectorQuantizerEMA(e_dim,n_e)
            ...
            if enforce_single_codebook_per_position and (
                dim / total_downproj != e_dim
            ):
                self.pre_quant_proj = torch.nn.Linear(dim // total_downproj, e_dim)
                self.post_quant_proj = torch.nn.Linear(e_dim, dim // total_downproj)
        ...
        print(
            f"Finished loading HPCT model with shorten factor {shorten_factor} and {256 // total_downproj} channel dimensions."
        )
